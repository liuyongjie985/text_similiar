...
def train(args):
    ...
    with tf.Session() as sess:
        ...
        for e in range(args.num_epochs):
            ...
            state1 = sess.run(model.initial_state1)
            state2 = sess.run(model.initial_state2)
            for b in range(data_loader.num_batches):
                start = time.time()
                x1, x2, y = data_loader.next_batch()
                y = np.split(y, args.batch_size, 0)
                feed = {model.input_data_1: x1, model.input_data_2: x2, model.targets: y}
                for i, (c, h) in enumerate(model.initial_state1):
                    feed[c] = state1[i].c
                    feed[h] = state1[i].h
                for i, (c, h) in enumerate(model.initial_state2):
                    feed[c] = state2[i].c
                    feed[h] = state2[i].h
                # instrument for tensorboard
